...
def query_all_dicts(param_dict):
    ...
    json_res = get_json_from_juchao(search_dict=query_dict)
    num_pages = get_num_pages_from_json(json_res)
    query_dict['pageNum'] = [i+1 for i in range(num_pages)]
    json_of_pages = get_json_from_juchao(search_dict=query_dict)
...
if __name__=='__main__':
    # 该字典为查询条件（设定为可自定义的部分），可以修改值value，但请勿更改关键字key
    param_dict = {
        # 搜索的关键词，如【'企业社会责任'】
        'searchkey':'社会及管治',
        # 发布起始时间，如【'2023-01-04'】【''表示不进行限制 】 
    ...
    SEARCH_RES_NAME = f"{'_'.join(param_dict.values())}_{l}_{r}"
    # 将结果转化为csv
